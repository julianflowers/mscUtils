...
coord <- str_match(deg, "(\\d{1,2})(◦|°|\\.||8?|:|\024.)?(\\D*)?(\\d{1,2}|\\d{1,2}′|\\d{1,2}\004).*([NSEWnsew])")
data.frame(coord)

str_match(deg[1], coord)
coord1 <- str_extract(deg, "[NSEWnsew].?(\\d{1,2})(◦|°|\\.||8|:).+")
coord1

eight_zero <- "(^\\d{1,2})(.*|8.*)(\\d{1,2}0.*|\\d{1,2}0.*\\d{1,2}00.*)[NSEWnsew]"
str_match(deg, eight_zero)


stringi::stri_enc_tonative(coord)
